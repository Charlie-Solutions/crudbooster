<div class='form-group form-datepicker {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}'
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        ...
        <div id='{{$name}}' class="input-group">
            <input id="1{{$name}}" type="hidden" name="{{$name}}" class="input-id" value="{{$value}}">
            <input id="2{{$name}}" type="text" class="form-control input-label {{$required?"required":""}}" {{$required?"required":""}} value="{{$datamodal_value}}" readonly>
            <span class="input-group-btn">
                ...
                <button style="background-color:rgb(235, 51, 0);" class="btn btn-primary" onclick="reset{{$name}}()" type="button"><i class='fa fa-info'></i>  Réinitialiser</button>
                <?php if(strlen($form['datamodal_module_path']) > 1){ ?>
                <a class="btn btn-info" href="{{CRUDBooster::adminPath()}}/{{$form['datamodal_module_path']}}" target="_blank">
                <i class='fa fa-edit'></i> {{$form['label']}}</a>
                <?php } ?>
            </span>
        </div><!-- /input-group -->
...
@push('bottom')
    <script type="text/javascript">
        ...
        function reset{{$name}}() {
            document.getElementById('1{{$name}}').value=0;
            document.getElementById('2{{$name}}').value=""; 
        }
    </script>
