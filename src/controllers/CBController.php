    ...
    public function getIndex()
    {
        ...
        foreach ($data['result'] as $row) {
            ...
            foreach ($columns_table as $col) {
                ...
                if (isset($col['image'])) {
                    if ($value == '') {
                        $value = "<a data-lightbox='roadtrip' rel='group_{{$table}}' title='$label: $title' href='".asset('vendor/crudbooster/avatar.jpg')."' target='_blank'><img width='40px' height='40px' src='".asset('vendor/crudbooster/avatar.jpg')."'/></a>";
                    } else {
...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
    ...
    public function postUploadSummernote()
    {
        $this->cbLoader();
        $name = 'userfile';
        if ($file = CRUDBooster::uploadFile($name, false)) {
            echo asset($file);
        }
    }
